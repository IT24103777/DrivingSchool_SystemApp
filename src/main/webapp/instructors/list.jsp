...
<div class="container mx-auto px-4 py-8">
  <div class="flex justify-between items-center mb-6">
    ...
    <div class="flex space-x-4">
      ...
      <a href="${pageContext.request.contextPath}/instructor-create"
  ...
  <div class="bg-white shadow rounded-lg overflow-hidden">
    <div class="overflow-x-auto">
      <table id="instructorTable" class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
        <tr>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Instructor Number</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Contact</th>
          <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
        </tr>
        </thead>
        <tbody id="instructorTableBody" class="bg-white divide-y divide-gray-200">
        <% for (InstructorModel instructor : instructors) { %>
        <tr data-id="<%= instructor.getId() %>" data-name="<%= instructor.getName() %>"
            data-number="<%= instructor.getInstructorNumber() %>" data-contact="<%= instructor.getContactNumber() %>"
            data-email="<%= instructor.getEmail() %>" data-nic="<%= instructor.getNicNumber() %>">
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><%= instructor.getId() %></td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><%= instructor.getName() %></td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><%= instructor.getInstructorNumber() %></td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><%= instructor.getContactNumber() %></td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
            ...
            <button onclick="confirmDelete(<%= instructor.getId() %>)"
...
<script>
  ...
  document.getElementById('searchInput').addEventListener('input', function() {
    ...
    rows.forEach(row => {
      ...
      if (name.includes(searchTerm) || number.includes(searchTerm) ||
              contact.includes(searchTerm) || email.includes(searchTerm) || nic.includes(searchTerm)) {
        row.style.display = '';
      } else {
  ...
  async function confirmDelete(id) {
    if (confirm('Are you sure you want to delete this instructor?')) {
      try {
        const response = await fetch('${pageContext.request.contextPath}/api/instructors/' + id, {
          method: 'DELETE'
        });
        ...
        if (response.ok) {
          ...
          showAlert(error.error || 'Failed to delete instructor', 'error');
        }
      } catch (error) {
